...
namespace HumanStoryteller.NewtonsoftShell.Newtonsoft.Json.Linq.JsonPath;
	...
	internal class JPath
	{
		...
		{
			return new JsonException("Unexpected character while parsing path query: " + _expression[_currentIndex].ToString());
		}
		...
		private bool TryParseValue(out object value)
		{
			...
			while (_currentIndex < _expression.Length)
			{
				...
				if (c == ' ' || c == ')')
				{
					...
					{
						long result;
						bool result2 = long.TryParse(text, NumberStyles.Integer, CultureInfo.InvariantCulture, out result);
						value = result;
						return result2;
					}
					double result3;
					bool result4 = double.TryParse(text, NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite | NumberStyles.AllowLeadingSign | NumberStyles.AllowDecimalPoint | NumberStyles.AllowThousands | NumberStyles.AllowExponent, CultureInfo.InvariantCulture, out result3);
					value = result3;
					return result4;
				}
				stringBuilder.Append(c);
