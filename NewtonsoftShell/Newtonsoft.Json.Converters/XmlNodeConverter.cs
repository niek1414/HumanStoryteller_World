	...
	public class XmlNodeConverter : JsonConverter
	{
		...
		private static string ConvertTokenToXmlValue(JsonReader reader)
		{
			...
			case JsonToken.Date:
			{
				...
				DateTime value3 = Convert.ToDateTime(reader.Value, CultureInfo.InvariantCulture);
				return XmlConvert.ToString(value3, DateTimeUtils.ToSerializationMode(value3.Kind));
			}
			case JsonToken.Bytes:
			...
			while (!flag && reader.Read())
			{
				...
				case JsonToken.PropertyName:
				{
					...
					if (!StringUtils.IsNullOrEmpty(text))
					{
						...
						case '$':
							if (text != null && (text == "$values" || text == "$id" || text == "$ref" || text == "$type" || text == "$value"))
							{
								...
								if (text2 == null)
								{
									...
									{
										num2 = num;
										if (manager.LookupNamespace("json" + num2.ToString()) == null)
										{
											break;
										}
				...
				case JsonToken.PropertyName:
				{
					...
					if (reader.TokenType == JsonToken.StartArray)
					{
						...
						if (num == 1 && WriteArrayAttribute)
						{
							...
							string b = StringUtils.IsNullOrEmpty(prefix) ? manager.DefaultNamespace : manager.LookupNamespace(prefix);
							foreach (IXmlNode childNode in currentNode.ChildNodes)
							{
								IXmlElement xmlElement = childNode as IXmlElement;
								if (xmlElement != null && xmlElement.LocalName == localName && xmlElement.NamespaceUri == b)
								{
									AddJsonArrayAttribute(xmlElement, document);
									break;
								}
