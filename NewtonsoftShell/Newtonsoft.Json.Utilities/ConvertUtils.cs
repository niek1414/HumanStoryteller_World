	...
	internal static class ConvertUtils
	{
		...
		public static ParseResult DecimalTryParse(char[] chars, int start, int length, out decimal value)
		{
				...
				if (num7 < 29)
				{
					if (num7 == 28)
					{
						...
						if (!flag3.HasValue)
						{
							...
							bool? flag4 = flag2;
							valueOrDefault = (flag4.GetValueOrDefault() ? 1 : 0);
						}
						else
						{
							valueOrDefault = (flag3.GetValueOrDefault() ? 1 : 0);
						}
						if (valueOrDefault != 0)
						{
							goto IL_01ff;
			...
			if (num4 > 0)
			{
				...
				if (num7 == 29)
				{
					...
					else if (value == 7922816251426433759354395033m)
					{
						int? num10 = c;
						int num11 = 53;
						if ((num10.GetValueOrDefault() > num11) & num10.HasValue)
						{
							return ParseResult.Overflow;
						}
					}
			...
			else
			{
				int? num10 = c;
				int num11 = 53;
				if (((num10.GetValueOrDefault() >= num11) & num10.HasValue) && num4 >= -28)
				{
					++value;
				}
				if (num4 < 0)
