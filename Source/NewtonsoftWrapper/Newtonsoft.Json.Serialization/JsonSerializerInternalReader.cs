	...
	internal class JsonSerializerInternalReader : JsonSerializerInternalBase
	{
			...
			if (reader.TokenType == JsonToken.PropertyName)
			{
				...
				do
				{
					...
					if (string.Equals(a, "$type", StringComparison.Ordinal))
					{
						...
						{
							ReflectionUtils.SplitFullyQualifiedTypeName(text2, out string typeName, out string assemblyName);
							Type type;
							try
							{
		...
		private void SetPropertyValue(JsonProperty property, JsonReader reader, object target)
		{
			...
			else
			{
				...
				else if (property.NullValueHandling.GetValueOrDefault(base.Serializer.NullValueHandling) == NullValueHandling.Ignore && reader.TokenType == JsonToken.Null)
				{
					reader.Skip();
				}
				else if (property.DefaultValueHandling.GetValueOrDefault(base.Serializer.DefaultValueHandling) == DefaultValueHandling.Ignore && JsonReader.IsPrimitiveToken(reader.TokenType) && object.Equals(reader.Value, property.DefaultValue))
				{
					reader.Skip();
				}
				else
		...
		private object CreateObjectFromNonDefaultConstructor(JsonReader reader, JsonObjectContract contract, string id)
		{
			...
			where !p.Ignored
			select p).ToDictionary((Func<JsonProperty, JsonProperty>)((JsonProperty kv) => kv), (Func<JsonProperty, object>)((JsonProperty kv) => null));
			bool flag = false;
			do
			{
				switch (reader.TokenType)
				...
				case JsonToken.PropertyName:
				{
					...
					if (closestMatchProperty != null)
					{
						...
						{
							dictionary[closestMatchProperty] = CreateValueProperty(reader, closestMatchProperty, null, gottenCurrentValue: true, currentValue: null);
						}
						else
						{
							reader.Skip();
		...
		private object PopulateObject(object newObject, JsonReader reader, JsonObjectContract contract, string id)
		{
			...
			Dictionary<JsonProperty, RequiredValue> dictionary = (from m in contract.Properties
			where m.Required != Required.Default
			select m).ToDictionary((JsonProperty m) => m, (JsonProperty m) => RequiredValue.None);
			if (id != null)
			{
