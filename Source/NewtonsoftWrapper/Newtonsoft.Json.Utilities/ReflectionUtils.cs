	...
	internal static class ReflectionUtils
	{
		...
		public static List<MemberInfo> GetFieldsAndProperties(Type type, BindingFlags bindingAttr)
		{
			...
			foreach (var item in enumerable)
			{
				...
				{
					IEnumerable<MemberInfo> collection = item.Members.Where(delegate(MemberInfo m)
					{
		...
		{
			ValidationUtils.ArgumentNotNull(targetType, "targetType");
			List<MemberInfo> list = new List<MemberInfo>(targetType.GetProperties(bindingAttr));
			GetChildPrivateProperties(list, targetType, bindingAttr);
			return list.Cast<PropertyInfo>();
		}
			...
			if ((bindingAttr & BindingFlags.NonPublic) != 0)
			{
				...
				while ((targetType = targetType.BaseType) != null)
				{
					...
					foreach (PropertyInfo propertyInfo in properties)
					{
						...
						int num = initialProperties.IndexOf((MemberInfo p) => p.Name == nonPublicProperty.Name);
						if (num == -1)
						{
