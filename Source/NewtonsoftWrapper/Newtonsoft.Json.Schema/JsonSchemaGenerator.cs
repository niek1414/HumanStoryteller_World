...
namespace Newtonsoft.Json.Schema
{
	...
	public class JsonSchemaGenerator
	{
		...
		public JsonSchema Generate(Type type, JsonSchemaResolver resolver, bool rootSchemaNullable)
		{
			...
			_resolver = resolver;
			return GenerateInternal(type, (!rootSchemaNullable) ? Required.Always : Required.Default, optional: false);
		}
		...
		private JsonSchema GenerateInternal(Type type, Required valueRequired, bool optional)
		{
			...
			else if (jsonContract is JsonDictionaryContract)
			{
				...
				{
					CurrentSchema.AdditionalProperties = GenerateInternal(valueType, Required.Default, optional: false);
				}
			}
			else if (jsonContract is JsonArrayContract)
			{
				...
				if (collectionItemType != null)
				{
					...
					CurrentSchema.Items.Add(GenerateInternal(collectionItemType, (!flag) ? Required.Always : Required.Default, optional: false));
				}
			}
		...
		private void GenerateObjectSchema(Type type, JsonObjectContract contract)
		{
			...
			foreach (JsonProperty property in contract.Properties)
			{
				if (!property.Ignored)
				{
					bool optional = property.NullValueHandling == NullValueHandling.Ignore || property.DefaultValueHandling == DefaultValueHandling.Ignore || property.ShouldSerialize != null;
					JsonSchema jsonSchema = GenerateInternal(property.PropertyType, property.Required, optional);
