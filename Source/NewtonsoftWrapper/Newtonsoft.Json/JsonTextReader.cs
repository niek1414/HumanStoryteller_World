...
namespace Newtonsoft.Json
{
	...
	public class JsonTextReader : JsonReader, IJsonLineInfo
	{
		...
		private bool ParseProperty(char firstChar)
		{
			...
			{
				c = MoveNext();
				EatWhitespace(c, oneOrMore: false, finalChar: out c);
				if (c != ':')
				{
		...
		private void ParseConstructor()
		{
			if (MatchValue('n', "new", noTrailingNonSeperatorCharacters: true))
			{
				...
				if (EatWhitespace(finalChar, oneOrMore: true, finalChar: out finalChar))
				{
					...
					EatWhitespace(finalChar, oneOrMore: false, finalChar: out finalChar);
					if (finalChar != '(')
					{
