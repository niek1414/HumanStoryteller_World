	...
	internal static class EnumUtils
	{
		...
		private static EnumInfo InitializeValuesAndNames( StructMultiKey<Type, NamingStrategy> key)
		{
			...
			for (int i = 0; i < names.Length; i++)
			{
				...
				object text2 = (from EnumMemberAttribute a in field.GetCustomAttributes(typeof(EnumMemberAttribute), inherit: true)
		...
		public static object ParseEnum(Type enumType,  NamingStrategy namingStrategy, string value, bool disallowNumber)
		{
			...
			ulong num3 = 0uL;
			int num4;
			for (int j = num2; j <= value.Length; j = num4 + 1)
			{
				num4 = value.IndexOf(',', j);
				if (num4 == -1)
				{
					num4 = value.Length;
				}
