...
namespace HumanStoryteller.NewtonsoftShell.Newtonsoft.Json.Utilities
{
	...
	internal class DynamicReflectionDelegateFactory : ReflectionDelegateFactory
	{
		...
		private void GenerateCreateMethodCallIL(MethodBase method, ILGenerator generator, int argsIndex)
		{
			...
			for (int i = 0; i < parameters.Length; i++)
			{
				...
				else if (parameterType.IsValueType())
				{
					...
					{
						MethodInfo method2 = typeof(IConvertible).GetMethod("To" + parameterType.Name, new Type[1]
						{
		...
		{
			DynamicMethod dynamicMethod = CreateDynamicMethod("Get" + propertyInfo.Name, typeof(object), new Type[1]
			{
				typeof(T)
