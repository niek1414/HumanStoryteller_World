...
namespace HumanStoryteller.NewtonsoftShell.Newtonsoft.Json.Serialization
{
	...
	internal class JsonSerializerInternalReader : JsonSerializerInternalBase
	{
		...
		private object CreateList( JsonReader reader, Type objectType, JsonContract contract, JsonProperty member, object existingValue, string id)
		{
			...
			goto IL_01ca;
			IL_01ca:
			return PopulateList((IList)list3, reader, jsonArrayContract, member, id);
		}
					...
					try
					{
						if (contract.IsConvertable)
						{
							...
							if (contract.IsEnum)
							{
								...
								if (text != null)
								{
									return EnumUtils.ParseEnum(contract.NonNullableUnderlyingType, null, text, disallowNumber: false);
								}
							...
							else if (contract.NonNullableUnderlyingType == typeof(DateTime))
							{
								...
								if (text2 != null && DateTimeUtils.TryParseDateTime(text2, reader.DateTimeZoneHandling, reader.DateFormatString, reader.Culture, out DateTime dt))
								{
		...
		private object PopulateDictionary(IDictionary dictionary, JsonReader reader, JsonDictionaryContract contract,  JsonProperty containerProperty,  string id)
		{
			...
			PrimitiveTypeCode primitiveTypeCode = (contract.KeyContract as JsonPrimitiveContract)?.TypeCode ?? PrimitiveTypeCode.Empty;
			bool flag = false;
			do
			{
				switch (reader.TokenType)
				...
				case JsonToken.PropertyName:
				{
					...
					if (!CheckPropertyName(reader, obj2.ToString()))
					{
						try
						{
							try
							{
								obj2 = (((uint)(primitiveTypeCode - 26) <= 1u) ? (DateTimeUtils.TryParseDateTime(obj2.ToString(), reader.DateTimeZoneHandling, reader.DateFormatString, reader.Culture, out DateTime dt) ? ((object)dt) : EnsureType(reader, obj2, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType)) : (((uint)(primitiveTypeCode - 28) > 1u) ? EnsureType(reader, obj2, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType) : (DateTimeUtils.TryParseDateTimeOffset(obj2.ToString(), reader.DateFormatString, reader.Culture, out DateTimeOffset dt2) ? ((object)dt2) : EnsureType(reader, obj2, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType))));
							}
							catch (Exception ex)
							{
				...
				if (stack.Count == arrayRank)
				{
					...
					catch (Exception ex)
					{
						...
						if (num.HasValue)
						{
							...
							if ((num2.GetValueOrDefault() == position2) & num2.HasValue)
							{
		...
		private object CreateObjectUsingCreatorWithParameters(JsonReader reader, JsonObjectContract contract,  JsonProperty containerProperty, ObjectConstructor<object> creator,  string id)
		{
			...
			if (flag)
			{
				...
				{
					if (!property3.Ignored && list.All((CreatorPropertyContext p) => p.Property != property3))
					{
						list.Add(new CreatorPropertyContext(property3.PropertyName)
						{
							Property = property3,
			...
			foreach (CreatorPropertyContext item in list)
			{
				if (flag && item.Property != null && !item.Presence.HasValue)
				{
					...
					PropertyPresence value2;
					if (value == null)
					{
						value2 = PropertyPresence.Null;
					}
					else
					{
						string text = value as string;
						value2 = ((text == null) ? PropertyPresence.Value : (CoerceEmptyStringToNull(item.Property.PropertyType, item.Property.PropertyContract, text) ? PropertyPresence.Null : PropertyPresence.Value));
					}
				...
				if (flag)
				{
					PropertyPresence? presence = item.Presence;
					PropertyPresence propertyPresence = PropertyPresence.None;
			...
			{
				if (!item2.Used && item2.Property != null && !item2.Property.Ignored)
				{
					PropertyPresence? presence = item2.Presence;
					PropertyPresence propertyPresence = PropertyPresence.None;
					if (!((presence.GetValueOrDefault() == propertyPresence) & presence.HasValue))
					{
							...
							else if (jsonContract.ContractType == JsonContractType.Dictionary)
							{
								...
								if (!jsonDictionaryContract.IsReadOnlyOrFixedSize)
								{
									...
									if (value5 != null)
									{
										...
										IDictionaryEnumerator enumerator4 = ((IDictionary)obj5).GetEnumerator();
										try
										{
			...
			if (contract.ExtensionDataSetter != null)
			{
				foreach (CreatorPropertyContext item4 in list)
				{
					if (!item4.Used)
					{
						PropertyPresence? presence = item4.Presence;
						PropertyPresence propertyPresence = PropertyPresence.None;
						if (!((presence.GetValueOrDefault() == propertyPresence) & presence.HasValue))
						{
		...
		{
			if (Serializer.MetadataPropertyHandling == MetadataPropertyHandling.ReadAhead && memberName != null && (memberName == "$id" || memberName == "$ref" || memberName == "$type" || memberName == "$values"))
			{
				reader.Skip();
				return true;
			}
		...
		private void SetPropertyPresence(JsonReader reader, JsonProperty property,  Dictionary<JsonProperty, PropertyPresence> requiredProperties)
		{
			...
			{
				JsonToken tokenType = reader.TokenType;
				PropertyPresence propertyPresence2 = requiredProperties[property] = ((tokenType == JsonToken.String) ? (CoerceEmptyStringToNull(property.PropertyType, property.PropertyContract, (string)reader.Value) ? PropertyPresence.Null : PropertyPresence.Value) : (((uint)(tokenType - 11) <= 1u) ? PropertyPresence.Null : PropertyPresence.Value));
			}
		}
