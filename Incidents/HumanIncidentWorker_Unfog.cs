...
class HumanIncidentWorker_Unfog : HumanIncidentWorker {
    ...
    protected override IncidentResult Execute(HumanIncidentParams @params) {
        ...
        if (!(@params is HumanIncidentParams_Unfog)) {
            Tell.Err("Tried to execute " + GetType() + " but param type was " + @params.GetType());
            return ir;
        }

        HumanIncidentParams_Unfog
            allParams = Tell.AssertNotNull((HumanIncidentParams_Unfog) @params, nameof(@params), GetType().Name);
...
public class HumanIncidentParams_Unfog : HumanIncidentParams {
    public bool Unfog;
